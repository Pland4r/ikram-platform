...
import { mockSessions, mockSubjects } from "@/lib/mock-data"
...
export function TeacherScheduleTab() {
  ...
  const sessionsByDay = dayNames.map((dayName, dayIndex) => ({
    day: dayName,
    dayIndex,
    sessions: teacherSessions.filter((session) => session.dayOfWeek === dayIndex),
  }))
                        ...
                        {sessions
                          ...
                          .map((session) => {
                            ...
                            return (
                              <div
                                key={session.id}
                                className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border"
                              >
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-7 gap-2">
                  {Array.from({ length: 35 }, (_, i) => {
                    ...
                    const daySessions = teacherSessions.filter((session) => session.dayOfWeek === dayIndex)

                    return (
                      <div
                        key={i}
                        className={`p-2 min-h-[80px] border rounded-lg ${
                          daySessions.length > 0 ? "bg-blue-50 border-blue-200" : "bg-gray-50"
                        }`}
                      >
                        <div className="text-sm font-medium text-gray-900 mb-1">{dayNumber}</div>
                        {daySessions.slice(0, 2).map((session) => (
                          <div key={session.id} className="text-xs bg-blue-100 text-blue-800 p-1 rounded mb-1">
                            {session.title}
                          </div>
                        ))}
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-blue-50 rounded-lg">
              <div className="text-2xl font-bold text-blue-600">{teacherSessions.length}</div>
              <div className="text-sm text-blue-800">Total Classes</div>
            </div>
            <div className="text-center p-4 bg-green-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">
                {new Set(teacherSessions.map((s) => s.subjectId)).size}
              </div>
              <div className="text-sm text-green-800">Different Subjects</div>
            </div>
